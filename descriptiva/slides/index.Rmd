...
<https://drive.google.com/drive/folders/1qsABfIyYxoSTERwVCSe6gOJxZcSvjEV1?usp=sharing>

- **.bg-red_light[Ejercicios y entregas]** de la asignatura
<https://drive.google.com/drive/folders/1R3edVN3GPNaY6WLXd1VBgEl2ses_wBty?usp=sharing>

- **.bg-yellow[Diapositivas de R]**
<https://dadosdelaplace.github.io/teaching/descriptiva/slides>

- **.bg-orange[Entregas de R]**: <https://github.com/dadosdelaplace/teaching/tree/main/descriptiva/eval>
...
#### [Bloque VIII: visualizando datos](#bloque-8)
...
# Ejercicio 3 de la hoja 2

> En un control de calidad se analiza el número de productos defectuosos de diferentes lotes, obteniendo los siguientes datos (justifica todas tus respuestas).
...
**.bg-purple_light[Resume los datos de manera numérica]** (lo que sea posible)
...
* Primer paso: **.bg-purple_light[generar la base de datos]** (con un tibble)
...
* Segundo paso: **.bg-purple_light[contar]**. ¿Cómo podemos contar (lo que conocemos como n o frecuencia absoluta)?
...
* Tercer paso: **.bg-purple_light[construir la tabla de frecuencias]**.
...
¿Cómo calcular la **.bg-purple_light[media, moda y mediana]**? La ventaja de usar `R` es que la tabla de frecuencias solo nos servirá para conocer un poco mejor los datos PERO **.bg-purple_light[no necesitamos la tabla para la mayoría de cálculos]**: lo haremos con las funciones de `R`
...
Para la **.bg-purple_light[moda]** deberemos extraer aquel que más se repite

```{r}
tabla_frec %>% slice_max(n, with_ties = TRUE) %>% pull(x)
```
...
¿Cómo calcular las **.bg-purple_light[medidas de dispersión]**?
...
Recuerda que los software nos devuelven la cuasi_algo: para comparar entre dos variables, nos servirá igualmente.
...
```{r}
datos %>% group_by(x <= 7) %>% count() %>%  ungroup()
```
...
> Calcula la mediana. Calcula los percentiles 15, 27, 61 y 93. Calcula los cuartiles
...
datos %>% 
  ...
  summarise(mediana = median(x),
            ...
            q1 = quantile(x, probs = 0.25), q3 = quantile(x, probs = 0.75))
```
...
# BLOQUE VIII: visualizando datos
