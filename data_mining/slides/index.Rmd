...
iris_rec <-
    ...
    add_role(starts_with("Petal"), new_role = "no simétrica") %>%
    # Detectar outliers
    step_mutate(across(has_role("simétrica"),
                       function(x) { ifelse(abs(scores(x, type = "z")) > 2.5, NA, x) }),
                across(has_role("no simétrica"),
                       function(x) { ifelse(abs(scores(x, type = "mad")) > 3, NA, x) })) %>%
    # Filtro de correlación
    step_corr(all_numeric_predictors(), threshold = 0.9)%>% 
    # Filtro de cero varianza
...
Probaremos 86 modelos diferentes

```{r}
grid_tree <-
  expand_grid("depth" = c(2, 3, 4, 5, 6, 7),
              "min_n" = c(0.01, 0.025, 0.05, 0.1) * nrow(iris_train),
              "cost" = c(0.01, 0.1, 0.5, 1))
grid_tree
```
...
```{r  eval = FALSE}
# Métricas
iris_fit_val_gini %>% collect_metrics()
```
...
```{r  eval = FALSE}
# Métricas
iris_fit_val_entropy %>% collect_metrics()
```

---

# .orange[VISUALIZACIÓN] del árbol

Con `rpart.plot()` podemos **.bg-purple_light[visualizar el árbol]** siempre y cuando el motor sea `"rpart"` (le indicamos que no redondee valores a enteros y de los modelos de visualización elegimos `extra = 4`, prueba con varios para ver las diferencias)
...
Con `vi()` (del paquete homónimo) podemos **.bg-purple_light[calcular la importancia relativa de las variables]**. Para detalles del cálculo ver <https://rdrr.io/cran/vip/man/vi_model.html> y <https://koalaverse.github.io/vip/articles/vip.html>

```{r eval = FALSE}
library(vip)
tabla_vip <- vi(iris_fit_val_gini)
tabla_vip
```
...
```{r eval = FALSE}
# Random forest con tune (y paralelizado)
rf_hotel <-
