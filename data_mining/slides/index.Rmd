...
#### [CLASE 3: INTRO A LA METODOLOGÍA SEMMA. TIDYDATA](#clase-3)
...
### [Introducción a minería: metodología SEMMA](#data-mining)
...
Instala el paquete `{tidyr}` y usa el conjunto `who` contenido en él mismo (sobre casos de tuberculosis). Lee la ayuda `? who` para detalles de los datos. Convierte a tidydata.
...
De lo general a la particular, **.bg-purple_light[¿qué es la ciencia de datos]** ¿Qué incluye?

La conocida como Data Science (Ciencia de Datos) es un campo muy extenso en el que, según algunos autores, se podría incluir (o intersecar con) campos como la **Minería de Datos**, el **Machine Learning** o el **Big Data**
...
Veamos un ejemplo: imagina que queremos construir un método que nos permita **.bg-purple_light[clasificar]** si un animal es un **.bg-purple_light[gato o perro]** en función de dos variables: **suavidad** y **peso**. En concreto el aprendizaje será **.bg-purple_light[supervisado]** (sé a priori en mi dataset cuál es gato o perro).
...
```{r echo = FALSE,  out.width = "145%", fig.align = "center"}
...
El **.bg-red_light[sesgo]** será por tanto lo que nos **.bg-red_light[equivocamos/desviamos de forma sistemática]** y la **.bg-green_light[varianza]** del modelo será la **.bg-green_light[dispersión entre las predicciones]** de un valor, como si repitieramos el modelo con distintas muestras aleatorias obtenidas de la misma población. 
...
En minería de datos tendremos habitualmente una variable con un rol especial
...
La metodología que se empleará en este proyecto será la metodología SEMMA. Es-
ta metodología fue propuesta por el SAS Institute, el cual la define como el proceso de

selección, exploración y modelado aplicado a cantidades significativas de datos almace-
nados que permitan el descubrimiento de patrones como herramientas de apoyo para el

negocio. Esta metodología está dividida en 5 fases detalladas a continuación:


2.2. Metodología
La metodología SEMMA fue propuesta por el SAS Institute con la finalidad de
esquematizar el proceso de minería de datos (SAS Institute, 2018). El nombre es un
acrónimo que recoge las iniciales de las siguientes cinco fases que propone esta técnica
(ver también Hernández y Dueñas, 2019):

* Sample (muestreo): extraer (si fuese necesario) un subconjunto representativo de nuestros datos con los que se realizará el estudio y, si fuese también necesario, extraer una
muestra representativa con el fin de reducir la dimensión de los datos y optimizar el
proceso de modelización.


- Explore (Exploración). Segunda fase de la metodología en la que se realiza un estudio
de los datos obtenidos a través de técnicas estadísticas, con el objetivo de entender
mejor el problema, detectar datos anómalos o ausentes, así como de inconsistencias
en la base de datos.
- Modify (Transformación). Tercera etapa del proceso en la que se modifican y tratan
los datos para solventar los problemas detectados en la etapa previa.

6
- Model (Modelización). Cuarta fase de la metodología SEMMA en la que se utilizan
técnicas y herramientas estadísticas o de aprendizaje automático con la finalidad de
descubrir similitudes en los datos y lograr una predicción/clasificación de la variable
objetivo (si la hubiera).
- Assess (Evaluación). Es la última etapa del proceso, en la que se debe valorar la
calidad del resultado obtenido en la fase de modelaje, haciendo uso de
metodologías de validación o de submuestras de validación o test, si fuese necesario.

Fig. 4: diagrama de las etapas de la metodología SEMMA. Fuente: Blogdatlas

A continuación se describen brevemente las técnicas utilizadas y los objetivos a
conseguir en cada una de las etapas de la metodología SEMMA en el trabajo.
2.2.1. Muestreo
En esta primera etapa de la metodología SEMMA se ha realizado la extracción de los
datos con los que se realizará la clasificación del retraso de los vuelos nacionales de
EE.UU. Para ello, se ha extraído información de diversas fuentes y se ha monitorizado su
obtención mediante la implementación de funciones de R, combinadas con diferentes
API y técnicas de web scrapping haciendo uso de Python. En un primer lugar se
obtuvieron 7 422 037 observaciones, pertenecientes a todos los vuelos nacionales en
Estados Unidos en el año 2019 (ver detalles y fuente en la Sección 3), pero debido a la
unión de diferentes fuentes de información y procesos de depuración, finalmente se
mantuvieron en la tabla 4 408 503 observaciones. Dicha reducción de la dimensión se
ha hecho imprescindible debido al alto volumen de datos y a la capacidad computacional
de los sistemas utilizados (toda la información sobre las funciones utilizadas se puede
consultar en el Anexo II). Como se indica además en futuras secciones, se han realizado
diferentes muestreos estratificados a posteriori, con el objetivo de reducir la
dimensionalidad del problema (y que pudiera ser abarcable con los medios informáticos
disponibles).

[aida]

Existen distintas metodologías/esquemas dentro de la minería de datos. Destacan la
metodología SEMMA (Sample-Explore-Modify-Model-Assess), desarrollada principalmente por
...
SAMPLE (muestrear): Si la base de datos es demasiado grande, será necesario tomar
una muestra lo suficientemente grande para contener toda la información y lo
suficientemente pequeña para poder ser procesada.
...
transformando variables para facilitar la modelización.
...
obtenidos.
No siempre intervienen todas las fases del proceso. Además, las fases pueden repetirse y el
