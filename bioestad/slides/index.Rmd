...
knitr::include_graphics("./img/dataviz_historico_1.webp")
``` 
...
No hay **.bg-purple_light[ningún PROCESO DE MEDIDA]** representado, no cuantifica nada.
...
El proceso de representación **.bg-purple_light[no es REVERSIBLE]** ni comparable (al menos no fácilmente): es una infografía, no un gráfico estadístico o dataviz.
...
2. Debe incluir **.bg-purple_light[información (datos)]** (medible o simulada),

3. La **.bg-purple_light[relación de representatividad]** debería ser **.bg-purple_light[reversible]**: los datos deberían poder «recuperarse» a partir de la gráfica (es un tipo particular de **.bg-orange[«biyección» matemática]**).
...
```{r echo = FALSE,  out.width = "99%", fig.align = "center"}
knitr::include_graphics("./img/sectores_muchas_categorías.webp")
``` 
...
```{r echo = FALSE,  out.width = "55%", fig.align = "center", fig.cap = "Ejemplo de metáfora visual mal ejecutada"}
...
* La figura elegida (una persona caminando) no guarda relación alguna con lo que se pretende representar: **.bg-purple_light[mala metáfora]**.
...
* La forma hace **.bg-purple_light[imposible la comparación entre áreas]**, amén de que la suma total supera el 100% (¿?)

* Sin aportar **fuente** de los datos.
...
```{r echo = FALSE,  out.width = "70%", fig.align = "center", fig.cap = "Ejemplo de una buena idea mal ejecutada"}
...
```{r echo = FALSE,  out.width = "100%", fig.align = "center", fig.cap = "Ejemplo de una buena idea mal ejecutada"}
knitr::include_graphics("./img/viz_fail_covid.jpg")
``` 
...
* Intenta usar dos ejes (a izquierda para mortalidad, a derecha para letalidad) pero no lo indica adecuadamente. Cuidado con los dos ejes: puede ser confuso

* Leyenda pequeña y muy mal referenciada

* Tamaño de la línea totalmente desproporcionada que **.bg-purple_light[impide la reversibilidad y la comparación]**

* Además compara celdas geográficas no comparables (por tamaño y contexto): ¿Galicia vs Alemania?
...
* La elección previa de la desagregación es incorrecta: áreas muy grandes que se podrían representar en una tabla de 9 filas

* Leyenda tan grande que atrae la atención más que el propio gráfico.
...
* En su momento se dijo que era novedosa: no lo es (la novedad tampoco debería ser un argumento a favor per se)

* **.bg-purple_light[No respeta la reversibilidad]**: es una ilustración, no una gráfica estadística. La forma de llevarlo a cabo infraestima los datos actuales y, por su **.bg-purple_light[distorsión de la realidad]**, no permite **medir** precisamente la estacional que se propone visualizar con coordenadas polares

* El **eje que recorre la espiral no aporta**, distrae de los datos a representar, **aumenta la distorsión producida** por la no linealidad de la espiral y duplica información.
...
El **.bg-purple_light[uso de colores]** no es solo algo estético: es una variable más, y la paleta elegida así como su uso es fundamental (por ejemplo, la inclusión de colores nos permite centrarnos en los oscuros, y comparar entre tonalidades), así como la **.bg-purple_light[escala de la geometría]** elegida.
...
```{r echo = FALSE, out.width = "90%", fig.align = "center", fig.cap = "Fuente: https://www.reddit.com/r/dataisbeautiful"}
knitr::include_graphics("./img/espirales_colores.webp")
``` 
...
```{r echo = FALSE, out.width = "78%", fig.align = "center", fig.cap = "Fuente: https://twitter.com/victimofmaths/status/1480550926066847749"}
...
knitr::include_graphics("./img/playfair_8.webp")
``` 
...
El paquete `{ggplot2}` se basa en la idea propuesta por Wilkinson en **.bg-purple_light[«Grammar of graphics»]**: dotar a los gráficos de una gramática propia. Una de las **.bg-purple_light[principales fortalezas]** de `R` no solo la **flexibilidad y rapidez** de `{tidyverse}`, también la **.bg-purple_light[visualización]** con el paquete `{ggplot2}`.
...
Veamos un **primer intento** para entender la filosofía. Imagina que queremos dibujar un **.bg-purple_light[scatter plot]** o **diagrama de dispersión** (de puntos).
...
# .orange[SCATTER PLOT]: .green[(x, y)]


¿Qué **.bg-purple_light[elementos necesitamos]** para realizar un diagrama de puntos?
...
* **.bg-purple_light[Mapeado (aes)]**: indicarle dentro de `aes()` (aesthetics) las variables en cada coordenada. Todo lo que esté **.bg-purple_light[dentro de aes() dependerá de los datos]**
...
Vamos a profundizar en ese mapeado: ¿cómo **.bg-purple_light[cambiar el rol]** de los ejes?
...
Es decir, el mapeado para que **enlace** coordenadas a datos será con `aes(y = gdpPercap, x = pop)`
...
¿Cómo se pintaría un scatter plot con la **esperanza de vida** en el eje X frente a la **renta per cápita**?
...
# Colores, tamaños y formas: .orange[constantes]

.pull-left[

Dentro de `geom_point()` tenemos varios argumentos a usar:

* `na.rm = ...`: si queremos que nos quite los ausentes antes de representar la variable.

* `show.legend = ...`: si queremos que se muestre o no en la leyenda (por defecto, no muestra si solo hay una capa).
...
* `colour = ...`: color (en el caso del punto se entiende que es algo sin dimensión, así que es el color del punto; en otros casos, será el color del contorno)

* `fill = ...`: color relleno (para objetos con relleno y contorno)
...
```{r echo = FALSE, out.width = "83%"}
ggplot(gapminder_1997, aes(y = gdpPercap, x = lifeExp)) +
  geom_point(color = "red", alpha = 0.3, shape = 5)
```
...
```{r echo = FALSE, out.width = "83%"}
ggplot(gapminder_1997, aes(y = gdpPercap, x = lifeExp)) +
  geom_point(color = "red", alpha = 0.5, shape = 22, fill = "black")
```
...
```{r echo = FALSE, out.width = "83%"}
ggplot(gapminder_1997, aes(y = gdpPercap, x = lifeExp)) +
  geom_point(color = "#2EA2D8", alpha = 0.5, size = 5)
```
...
# .orange[SCATTER PLOT]: .green[aes(size, color, shape)]

.pull-left[

Hasta ahora los **parámetros estéticos** se los hemos pasado fijos y **constantes**.  Pero la verdadera potencia y versatilidad de `ggplot` es que podemos **.bg-purple_light[mapear los atributos estéticos]** en `aes()` para que dependan de variables de los datos

&nbsp;

Por ejemplo, vamos a asignar un **.bg-purple_light[color a cada dato en función de su continente]**.
...
* **.bg-purple_light[transparencia]** la fijamos  **.bg-orange[constante]** del 70%.
...
Usando los datos hemos conseguido dibujar en un **.bg-purple_light[gráfico bidimensional 4 variables]**: `lifeExp` y `gdpPercap` en los ejes $(X, Y)$), `continent` como color y `pop` como tamaño de la geometría, con muy pocas líneas de código.
...
# CLASE 16: profundizando en ggplot
