...
* **.bg-purple_light[Entrega I]**: contará un **.bg-orange[15% de la nota]** (sobre 100%, deadline 20 de noviembre)

* **.bg-purple_light[Entrega II]**: contará un **.bg-orange[35% de la nota]** (sobre 100%, deadline 11 de diciembre)

* **.bg-purple_light[Entrega III]**: contará un **.bg-orange[50% de la nota]** (deadline, 25 de diciembre)
...
#### [CLASE 2: introducción a tidyverse](#clase-2)
...
.panel[.panel-name[Vectores: II]

* 📝 **Ejercicio 4**: usando la fecha de hoy, construye un vector de tipo fecha de longitud 7 (que incluya hoy, los 3 días anteriores y los 3 días posteriores). Tras ello, extrae el día de cada una de las fechas.
 
* 📝 **Ejercicio 5**: echa un vistazo al material complementario. Diseña un bucle `for` de 200 iteraciones que, empezando en un valor inicial de 100 (euros), te sume 3€ si el número actual de la iteración es par y te reste 5€ si la iteración es impar (un número par será todo aquel número que al dividir entre 2, la división es exacta, es decir, que su resto es nulo; para calcular ese resto usaremos la función `%%`, por ejemplo `5 %% 2`).
...
Deberás **.bg-purple_light[entregar completado]** el archivo `.Rmd` que tienes disponible en <https://github.com/dadosdelaplace/teaching/tree/main/data_science_seguros/eval/ENTREGA%201> **.bg-purple_light[antes de las 23:59 del 20 de noviembre ]**, siguiendo las instrucciones contenidas en el mismo.
...
#### [Ejercicios extras](#ejercicios-II)
...
La entrega de dicho ejercicios tienen de deadline antes de la próxima clase y son **.bg-purple_light[voluntarios]** (no contará para la nota). A cambio el alumno que lo haga recibirá una corrección de la tarea.

.panelset[

.panel[.panel-name[Matrices]

* 📝 **Ejercicio 1**:  el siguiente código define una matriz de dimensiones `4 x 3` y calcula la suma por columnas. Modifica el código para que realice la suma por filas.
...
* 📝 **Ejercicio 2**: tomando la matriz del ejercicio anterior, accede a la segunda variable. Accede al primer registro. Accede al valor de la tercera variable del segundo individuo.
...
.panel[.panel-name[Tidy data I]

* 📝 **Ejercicio 1**: del paquete `{tidyr}` carga el dataset `oms` y busca en la ayuda (`? oms`) para ver que significan las variables (datos relacionados con casos de tuberculosis **desagregados por año, país, edad, género y el tipo**). Tras analizar la tabla, construye partir de ella una nueva con los mismos datos (no borres nada) pero recolocados de tal forma que sea tidydata
...
.panel[.panel-name[Tidy data II]

* 📝 **Ejercicio 2**: repite el ejercicio anterior pero eliminando las filas que tengan casos ausentes.


* 📝 **Ejercicio 3**: echa un vistazo a la ayuda de `count()` y aplica dicha función para contar cuantos registros tienes de cada tipo de enfermedad.
...
# .orange[ENTREGA II (35%)]

Carga el entorno de paquetes `{tidyverse}` y usa los conjuntos de datos `billboard` y `who`. Deberás **.bg-purple_light[entregar completado]** el archivo `.Rmd` que tienes disponible en <https://github.com/dadosdelaplace/teaching/tree/main/data_science_seguros/eval/ENTREGA%202> **.bg-purple_light[antes de las 23:59 del 11 de diciembre]**, siguiendo las instrucciones contenidas en el mismo.
...
# CLASE 3: profundizando en tidyverse

&nbsp;

### [Creando variables](#mutate)

### [Contar y agrupar](#count_group)

### [Resumir](#summarise)

### [Introducción al dataviz](#dataviz-intro)

### [ggplot2](#intro-ggplot2)
...
# Dataviz: .orange[INTRODUCCIÓN]

La aparición de **.bg-purple_light[gráficos estadísticos es relativamente reciente]** en la ciencia (hasta la Edad Media solo se acercaban a la idea los mapas)


Podríamos considerar los **.bg-purple_light[mapas]** como las primeras visualización de dato, con la representación de nuestra realidad en superficies bidimensionales. Las propias palabras _chart_ y _cartography_ derivan del mismo origen latino, _charta_.
...
# Navegación y .orange[ASTRONOMÍA]

Es en la época medieval, en la que la **.bg-purple_light[navegación y la astronomía]** empezaban a tomar relevancia, cuando aparece la que se considera la primera gráfica (aunque no propiamente estadística) <sup>3</sup>, representando el **.bg-purple_light[movimiento cíclico de los planetas]** (siglos X y XI)

```{r echo = FALSE,  out.width = "57%", fig.align = "center", fig.cap = "Gráfica extraída de Beniger y Robyn (1978)"}
...
Con una motivación similar, en torno a 1360 el matemático **Nicole Oresme** diseñó el **.bg-purple_light[primer gráfico de barras]** (no estadístico), con la idea de **visualizar a la vez dos magnitudes** físicas teóricas.
...
La mayoría de expertos, como Tufte <sup>6,7</sup>, consideran este gráfico como la **.bg-purple_light[primera visualización de datos]** de la historia, hecha por **Van Langren** en 1644, representando la **.bg-purple_light[distancia (en longitud) entre Toledo y Roma]**

.pull-left[

```{r echo = FALSE,  out.width = "90%", fig.align = "center", fig.cap = "Gráfica original extraída de Friendly y Valero-Mora (2010)"}
...
```{r echo = FALSE,  out.width = "90%", fig.align = "center", fig.cap = "Adaptación extraída de Friendly y Valero-Mora (2010)"}
...
1. Que se base en el esquema de composición de **.bg-purple_light[eje métrico]**

2. Debe incluir **.bg-purple_light[información cuantitativa]** (debe visualizar datos)

3. La relación de representatividad debería **.bg-purple_light[ser reversible]**: los **datos deberían poder «recuperarse»** a partir de la gráfica (una gráfica estadística es un tipo particular de aplicación matemática).
...
Hay muchas formas de hacer una gráfica estadística, y **no suele pasar** por hacer un gráfico de tartas. Los **.bg-purple_light[diagramas de tartas o sectores tiene un problema]** de reversibilidad:
...
Algo similar sucede con los mal llamados **.bg-purple_light[gráficos tridimensionales]** (son bidimensionales con perspectiva en realidad): los valores más cercanos aparecen sobredimensionados, siendo prácticamente imposible la reversibilidad.
...
```{r echo = FALSE, out.width = "80%", fig.align = "center", fig.cap = "Gráficas de Playfair, extraídas de Funkhouser y Walker (1935)"}
...
```{r echo = FALSE, out.width = "30%", fig.align = "center", fig.cap = "Gráficas de Playfair, extraídas de Funkhouser y Walker (1935)"}
...
Playfair no solo fue el primero en usar el dataviz para entender la realidad: fue el primero en usar **conceptos modernos** como el _grid_ o el color como un elemento no solo estético.

.pull-left[

```{r echo = FALSE, out.width = "80%", fig.align = "center", fig.cap = "Gráficas de Playfair, extraídas de https://friendly.github.io/HistDataVis"}
...
```{r echo = FALSE, out.width = "80%", fig.align = "center", fig.cap = "Gráficas de Playfair, extraídas de la wikipedia."}
...
# Primer .orange[GRÁFICO DE BARRAS]
...
```{r echo = FALSE, out.width = "90%", fig.align = "center", fig.cap = "Gráficas de Playfair de importaciones (barras grises) y exportaciones (negras) de Escocia en 1781"}
...
```{r echo = FALSE, out.width = "90%", fig.align = "center", fig.cap = "Primer diagrama de barras (Philippe Buache y Guillaume de L’Isle), visualizando los niveles del Sena desde 1732 hasta 1766"}
...
```{r echo = FALSE, out.width = "70%", fig.align = "center", fig.cap = "Gráfica de Playfair, visualizando 3 series temporales"}
...
```{r echo = FALSE, out.width = "70%", fig.align = "center", fig.cap = "Gráfica de Playfair, visualizando un time-line histórico"}
...
# .orange[MAPAS DE MINARD]

Minard fue el autor del famoso «Carte figurative des pertes successives en hommes de l'Armée Française dans la campagne de Russie 1812-1813», según Tufte «el mejor gráfico estadístico jamás dibujado», publicado en 1869 sobre la **.bg-purple_light[desastrosa campaña rusa de las tropas de Napoleón en 1812]**, mostrando 3 variables en un gráfico bidimensional

```{r echo = FALSE, out.width = "70%", fig.align = "center", fig.cap = "Gráfico de Minard sobre el avance de las tropas de Napoleón hacia Rusia, extraída de https://friendly.github.io/HistDataVis."}
...
# .orange[Francis Galton]

.pull-left[

Primo de Charles Darwin, inventor de los silbatos para perretes, de los mapas de predicción meteorológica y la persona que acuñó el concepto de **.bg-purple_light[regresión]** (y el de eugenesia :/).
...
# Diagrama de .orange[ÁREA POLAR]

Se le atribuye a Farr el primer **.bg-purple_light[diagrama de área polar o radial]**, cambiando por completo el **paradigma cartesiano** de la época: para visualizar la estacionalidad de la mortalidad en función de la temperatura, Farr decidió visualizarlo con coordenadas polares.
...
* El 21 de octubre de 1854 **.bg-purple_light[Florence Nigthingale]** fue enviada para mejorar las condiciones sanitarias de los **.bg-purple_light[soldados británicos]** en la guerra de Crimea. 

* A su regreso se dedicó a demostrar que los **.bg-purple_light[soldados fallecían por las condiciones sanitarias]**: eran muertes evitables.

* Nigthingale es la creadora del famoso **.bg-purple_light[diagrama de rosa]**, permitiendo pintar tres variables a la vez y su estacionalidad.

* El 8 de febrero de 1955, The Times la describió como la **.bg-purple_light[«ángel guardián» de los hospitales]**, y al finalizar la contienda, fue recibida como una heroína, conocida como **«The Lady with the Lamp»** tras un poema de H. W. Longfellow publicado en 1857.
...
```{r echo = FALSE, out.width = "65%", fig.align = "center", fig.cap = "Infografía extraída de https://twitter.com/storywithdata"}
...
# Introducción a .orange[ggplot2]


El paquete `{ggplot2}` se basa en la idea propuesta en **.bg-purple_light[«Grammar of graphics»]** del recién fallecido Wilkinson (de ahí el gg)


.pull-left[

```{r echo = FALSE,  out.width = "60%", fig.align = "center", fig.cap = "Imagen extraída de Reddit"}
...
# ggplot2: .orange[GRAMMAR OF GRAPHICS] 

.pull-left[

La idea de la filosofía detrás de `{ggplot2}` es entender los **.bg-purple_light[gráficos como parte integrada del flujo de datos]**, dotándoles de una gramática, basándose en la idea de 
[«The Grammar of Graphics» de Leland Wilkinson](https://www.amazon.es/Grammar-Graphics-Statistics-Computing/dp/0387245448).

El objetivo es empezar con un lienzo en blanco e ir **.bg-purple_light[añadiendo capas a tu gráfico]**, como harías por ejemplo en Photoshop, con la diferencia de que nuestras capas podemos **.bg-purple_light[vincularlas a los datos]**.

La ventaja de `{ggplot2}` es poder **.bg-purple_light[mapear atributos estéticos]** (color, forma, tamaño) de objetos geométricos (puntos, barras, líneas) en función de los datos.
...
La **.bg-purple_light[documentación]** del paquete puedes consultarla en <https://ggplot2-book.org/introduction.html> 
...
* **.bg-purple_light[Datos]**
* **.bg-purple_light[Mapeado]** de elementos estéticos (aesthetics): ejes, color, forma, tamaño, etc (en función de los datos)
* **.bg-purple_light[Elementos geométricos]** (geom): puntos, líneas, barras, polígonos, etc.
* **.bg-purple_light[Componer gráficas]** (facet): visualizar varias gráficas a la vez.
* **.bg-purple_light[Transformaciones]** estadísticas (stat): ordenar, resumir, agrupar, etc.
* **.bg-purple_light[Sistema de coordenadas]** (coord): coordenadas, grids, etc.
...
Veamos un **primer intento** para entender la filosofía. Imagina que queremos dibujar un **.bg-purple_light[scatter plot]** o **.bg-purple_light[diagrama de (dispersión)]** de puntos.
...
* **.bg-purple_light[Eje X]**: renta per cápita (variable `gdpPercap`)
* **.bg-purple_light[Eje Y]**: población (variable `pop`)

**¿Qué necesitamos?**

* **Datos**:  `gapminder_1997`.
* **Mapeado**: indicarle dentro de `aes()` (aesthetics) las variables a pintar en cada coordenada. Todo lo que esté **.bg-purple_light[dentro de aes() dependerá de los datos]**.
...
# .orange[COLORES], .green[TAMAÑOS] y formas
...
# Gráfico .orange[MULTIDIMENSIONAL]
...
# .orange[COORDENADAS] y .green[TEMA]
